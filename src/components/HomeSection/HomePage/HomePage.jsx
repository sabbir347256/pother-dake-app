...
const HomePage = () => {
    ...
    const { data: driverResponse, isLoading: isDriverLoading, refetch } = useQuery({
        queryKey: ["topDriversHomePage"],
        queryFn: async () => {
            const res = await axios.get(`${config?.backendUrl}/user/drivers`, {
                headers: {
    ...
    const topDrivers = driverResponse?.data || driverResponse || [];
    ...
    const getCurrentLocation = async () => {
        try {
            ...
            if (geocode && geocode.length > 0) {
                ...
                const formattedAddress = `${item.name || item.street || ""}, ${item.city || item.district || item.subregion || ""
                    }`;
            ...
            setLocationName("Dhanmondi, Dhaka");
        } finally {
            setIsLocLoading(false);
        }
    };
            ...
            <View className="pt-8 px-5 pb-6 bg-[#0F172A] rounded-b-[36px] border-b border-slate-800/80">
                <View className="flex-row justify-between items-center mb-6">
                    <View>
                        ...
                        <Text className="text-white text-2xl font-extrabold mt-0.5">
                            {role === "driver" ? "Ready to Drive?" : "Ready to Ride?"}
                        </Text>
                    </View>
                    <View className="flex-row items-center bg-emerald-950/60 border border-emerald-500/30 px-3 py-1.5 rounded-full">
                        ...
                        <Text className="text-emerald-400 text-xs font-bold ml-1.5">
                            {role === "driver" ? "Verified Driver" : "Safe Verified"}
                        </Text>
                    </View>
                </View>
                ...
                <View className="items-center justify-center my-4 h-52 relative">
                    ...
                    >
                        <Svg width="200" height="120" viewBox="0 0 200 120" fill="none">
                            ...
                            <Path
                                d="M 180 72 Q 188 72 185 78 Q 180 80 178 75 Z"
                                fill="#F59E0B"
                            />
                        </Svg>
                    </Animated.View>
                ...
                <View className="bg-[#1E293B]/90 border border-slate-700/60 rounded-2xl p-3 flex-row items-center">
                    <TouchableOpacity
                        ...
                        className="w-8 h-8 rounded-full bg-emerald-500/20 items-center justify-center mr-3"
                    >
                        {isLocLoading ? (
                    ...
                    {role === "driver" ? (
                        <>
                            <TouchableOpacity
                                activeOpacity={0.8}
                                onPress={() => router.push("/create")}
                                className="w-[45%] bg-[#111827] border border-slate-800 rounded-2xl p-4 items-center"
                            >
                            ...
                            >
                                <View className="w-12 h-12 bg-teal-950 rounded-full items-center justify-center mb-2">
                                    <ListOrdered size={22} color="#14B8A6" />
                                </View>
                                <Text className="text-white font-medium text-xs text-center">
                                    Requests
                                </Text>
                ...
                {role === "passenger" ? (
                    <>
                        ...
                        {isDriverLoading ? (
                                        ...
                                        <View className="flex-row items-center">
                                            <View className="w-12 h-12 bg-slate-700 rounded-full items-center justify-center mr-3">
                                                <Text className="text-white font-bold text-base">
                                                    {getInitials(driver.name || driver.fullName)}
                                                </Text>
                                            </View>
                                            <View>
                                                ...
                                                <Text className="text-slate-400 text-xs">
                                                    {driver.vehicleType || "Verified Vehicle"} •{" "}
                                                    {driver.phone || "Active"}
                                                </Text>
                                            </View>
                                        </View>
                            ...
                            <View className="bg-[#111827] border border-slate-800 rounded-2xl p-4 items-center">
                                <Text className="text-slate-400 text-xs">
                    ...
                    <>
                        <View className="flex-row justify-between items-center mb-4">
                            <Text className="text-white text-lg font-bold">Driver Dashboard</Text>
                        </View>
                        ...
                        >
                            <View className="flex-row items-center">
                                <View className="w-12 h-12 bg-emerald-950 rounded-full items-center justify-center mr-3">
                                ...
                                <View>
                                    ...
                                    <Text className="text-slate-400 text-xs">
                                        Check who requested your rides
                                    </Text>
                                </View>
                            </View>
                        </TouchableOpacity>
